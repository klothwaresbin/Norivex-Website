...
type TurnstileApi = { render: (element: HTMLElement, options: { sitekey: string; theme?: string; size?: string; callback?: (token: string) => void; 'expired-callback'?: () => void; 'error-callback'?: () => void }) => string };
...
function AssessmentForm() {
  ...
  useEffect(() => {
    ...
    fetch('/api/turnstile-config', { cache: 'no-store' })
      ...
      .catch(() => {
        if (!cancelled) setSubmitError('Bot verification could not load. Please refresh the page and try again.');
      });
    return () => { cancelled = true; };
  }, []);

  useEffect(() => {
    if (!turnstileSiteKey || !turnstileScriptReady || !turnstileContainerRef.current || turnstileRenderedRef.current) return;
    ...
    turnstile.render(turnstileContainerRef.current, {
      ...
      'error-callback': () => {
        setTurnstileToken('');
        setSubmitError('Bot verification could not load. Please refresh the page and try again.');
      },
    });
